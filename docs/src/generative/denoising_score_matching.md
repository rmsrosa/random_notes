...
Here, we explore the **denoising score matching** method proposed by [Pascal Vincent (2011)](https://doi.org/10.1162/NECO_a_00142) and illustrate it by fiting a multi-layer perceptron to model the score function of a one-dimensional synthetic Gaussian-mixture distribution.
...
## Loss functions for denoising score matching
...
```
where $C$ is constant with respect to the parameters, so we only need to minimize the **implicit score matching** objetive $\tilde J$, given by
```math
...
A few years later, [Pascal Vincent (2011)](https://doi.org/10.1162/NECO_a_00142) explored the idea (mentioned *en passant* in [Aapo Hyvärinen (2005)](https://jmlr.org/papers/v6/hyvarinen05a.html)) of using Parzel kernel density estimation
```math
    \tilde p_\sigma(x) = \int K\left(\frac{x - x_n}{\sigma}\right) \;\mathrm{d}\tilde p_0(x) = \frac{1}{\sigma N}\sum_{n=1}^N K\left(\frac{x - x_n}{\sigma}\right),
```
where $\sigma > 0$ is a kernel window parameter, $K(x)$ is a kernel density (properly normalized to have mass one) and $\tilde p_0(x)$ is the **empirical distribution** given by
```math
    p_0(x) = \frac{1}{N}\sum_{n=1}^N \delta(x - x_n).
```
In this way, the implicit score matching objetive function is approximated by
```math
    {\tilde J}_{\mathrm{ISM_\sigma}}({\boldsymbol{\theta}}) = \int_{\mathbb{R}} p_{\mathbf{X}}(\mathbf{x}) \left( \frac{1}{2}\left\|\boldsymbol{\psi}(\mathbf{x}; {\boldsymbol{\theta}})\right\|^2 + \boldsymbol{\nabla}_{\mathbf{x}} \cdot \boldsymbol{\psi}(\mathbf{x}; {\boldsymbol{\theta}}) \right)\;\mathrm{d}\mathbf{x},
```
Computing the score function of the Parzen estimation is still a bit expensive, so ...
...
function loss_function_kse(model, ps, st, data)
    sample_points, y_score_kse = data
    y_score_pred, st = Lux.apply(model, sample_points, ps, st)
    loss = mean(abs2, y_score_pred .+ y_score_kse) / 2
    return loss, st, ()
end
```
