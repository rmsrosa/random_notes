...
4. Markov-Chain-Monte-Carlo methods to generate samples when we only know a multiple $f(x)$ or the energy $U(x)$ of the PDF, i.e. $p(x) = f(x)/Z$ or $p(x) = e^{-U(x)}/Z,$ for an unknown or hard-to-compute normalizing constant $Z;$
5. Langevin sampling when we only know the Stein score $s(x) = \nabla\ln p(x)$ of the distribution.